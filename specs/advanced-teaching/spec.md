...
> **已废弃（cc-assistant-v3）**：本 spec 描述 v2 的「按需进阶能力教学」。v3「模块化上手引导课程」已取代：进阶能力成为独立课程模块（Skills/子智能体/Hooks/MCP/Headless/Agent SDK/Plugins，见 `module-course-orchestration` 模块清单），两阶段教学见 `two-phase-teaching`。不再作为活动需求，保留作历史记录。
