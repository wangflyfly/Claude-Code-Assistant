...
4. **独立复现验证**：交还控制权，只观察与核对结果，不代做。学习者卡住时先引导其自己尝试，必要时才给最小提示。结果不符合预期 → 回到相关教学点重讲或给最小提示再试；仍失败 → 告知可用 `/help`、`claude-code-guide`、官方文档继续学习，不强行判定失败。
...
- 真实项目有未提交改动：先建议 commit 或备份，再继续操作。
- **学习者决定权**：演示/讲解可以，实际落地（如写入配置）由学习者决定，不代行决策。

## 验证

- 成功标准：教学闭环后，学习者不靠引导独立完成一个小任务，且结果符合预期。
