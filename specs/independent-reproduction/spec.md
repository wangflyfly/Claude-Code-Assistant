...
> **已废弃（cc-assistant-v3）**：本 spec 描述 v2 的「教学闭环后独立复现」成功标准。v3「模块化上手引导课程」已取代：独立复现扩展为每模块真实轻练习独立完成（`per-module-exercise`）+ 收官整合综合任务（`integration-capstone`）。不再作为活动需求，保留作历史记录。
